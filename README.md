...
### Score confidence for all existing mappings

If you want a confidence score for **every** row in `claim_superclaim_map.json` (NC_* → SC_*), run:

```bash
cd src/dashboard/backend
python score_all_mappings.py --taxonomy-dir .. --out claim_superclaim_map_confidence.jsonl
```

This writes one JSON object per line with:

- `subclaim_id`, `subclaim_text`
- `superclaim_id`, `superclaim_text`
- `verdict`, `confidence`, `reason`, `model`

Set `OPENAI_API_KEY` (and optionally `OPENAI_MODEL`) in `src/dashboard/backend/.env` or your environment.
